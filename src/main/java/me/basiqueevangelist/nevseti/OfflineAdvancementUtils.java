...
public final class OfflineAdvancementUtils {
    ...
    public static void grant(UUID uuid, Advancement advancement) {
        Map<Identifier, AdvancementProgress> map = copyAdvancementMap(
                me.basiqueevangelist.nevseti.OfflineAdvancementCache.INSTANCE.get(uuid));
        AdvancementProgress progress = getProgress(map, advancement);
        for (String criterion : progress.getUnobtainedCriteria()) {
            progress.obtain(criterion);
        }
        me.basiqueevangelist.nevseti.OfflineAdvancementCache.INSTANCE.save(uuid, map);
    }
